...
unsigned char CFG_LSB, CFG_MSB,TRI_LSB,TRI_MSB, PTAT_LSB, CPIX_LSB, CPIX_MSB, PIX_LSB, PIX_MSB;
int PIX, v_th, CPIX;
...
int a_ij[64];
int b_ij[64];
unsigned char EEPROM_DATA[256];                   // Arreglo con capacidad para 256 bytes. Contenido de los datos de la memoria EEPROM
...
void write_trimming_value(unsigned char val){        // Funcion que escribe el valor del oscilador. Valor contenido el la EEPROM predeterminado por MELEXIS
    ...
    i2c_write((unsigned char)val-0xAA);               // Se envia el LSByte check
...
void config_MLX90620_Hz(int Hz){            // Aqui se seleccionan el valor del LSByte dependiendo de valor de Hz
    ...
    i2c_write((unsigned char)Hz_LSB-0x55);             // Se envia el LSByte check
